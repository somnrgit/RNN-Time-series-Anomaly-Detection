...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 判断GPU能否被PyTorch调用
# 1、创建解释器-使用argparse的第一步是创建一个ArgumentParser对象
...
parser.add_argument('--data', type=str, default='hekang',
                    help='type of the dataset (ecg, gesture, power_demand, space_shuttle, respiration, nyc_taxi')
parser.add_argument('--filename', type=str, default='fbox_hdata_0707.pkl',
...
# 3、解析参数-ArgumentParser通过parse_args()方法解析参数
...
# TimeseriesData = preprocess_data.PickleDataLoad(data_type='hekang', filename='fbox_hdata_0704.pkl',
...
def generate_output(args, epoch, model, gen_dataset, disp_uncertainty=True, startPoint=500, endPoint=3500):  # 生成输出
    if args.save_fig:
...
def evaluate_1step_pred(args, model, test_dataset):  # 评估每步的精度
    # Turn on evaluation mode which disables dropout.
    model.eval()  # 启用评估模式，不启用 BatchNormalization 和 Dropout
    total_loss = 0
    with torch.no_grad():
...
def train(args, model, train_dataset, epoch):  # 训练模式
    print("this is train_module of train_predictor")
    with torch.enable_grad():  # 启用梯度计算
        ...
        for batch, i in enumerate(range(0, train_dataset.size(0) - 1, args.bptt)):
            ...
            optimizer.zero_grad()  # 因为如果不清零，那么使用的这个grad就得同上一个mini-batch有关

            '''Loss1: Free running loss''' # 计算一个epoch的损失，最后查看的指标是  runningloss/样本数
            ...
            # modified by bruce @220620, start
            ...
            loss.backward()  # optimizer更新参数空间需要基于反向梯度

            # `clip_grad_norm` helps prevent the exploding gradient problem in RNNs / LSTMs.
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip)
            optimizer.step()  # mini-batch训练模式是假定每一个训练集就只有mini-batch这样大，因此实际上可以将每一次mini-batch看做是一次训练
            ...
            if batch % args.log_interval == 0 and batch > 0:
                ...
                print('| epoch {:3d} | {:5d}/{:5d} batches | ms/batch {:5.4f} | '
                      'loss {:5.2f} '.format(epoch, batch, len(train_dataset) // args.bptt, elapsed * 1000 / args.log_interval, cur_loss))
                total_loss = 0
                start_time = time.time()


def evaluate(args, model, test_dataset):  #评估模式
    # Turn on evaluation mode which disables dropout.
    print("this is evaluate_module of train_predictor")
    model.eval()
    with torch.no_grad():
        total_loss = 0
        hidden = model.init_hidden(args.eval_batch_size)
        ...
        for nbatch, i in enumerate(range(0, test_dataset.size(0) - 1, args.bptt)):
            ...
            # modified by bruce @220620, start
            # loss3 = criterion(hids1.view(args.batch_size,-1), hids2.view(args.batch_size,-1).detach())
            loss3 = criterion(hids1.contiguous().view(args.batch_size, -1),
                              hids2.contiguous().view(args.batch_size, -1).detach())
            # modified by bruce @220620, end
